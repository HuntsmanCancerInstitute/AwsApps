...
public class Email{
	...
	public Email(String smtpHostName, String smtpUser, String smtpPassword, String MIMEContentType){
		...
		if (MIMEContentType != null) contentType = MIMEContentType;
	}
	
	public boolean loadEmailInfo(File configFile) {
		
		/*aws_access_key_id
		 to_email_addresses david.austin.nix@gmail.com
		 smtp_hostName smtp.gmail.com
		 smtpUser david.austin.nix
		 smtpPassword larry2Tina
		 from_email_address doNotReply@gobama.org
		 */
		String[] lines = Util.loadTxtFile(configFile);
		
		for (String l : lines) {
			l = l.trim();
			String[] tokens = Util.WHITESPACE.split(l);
			if (tokens.length != 2) continue;
			if (l.startsWith("email")) emailAddresses = Util.COMMA.split(tokens[1]);
			else if (l.startsWith("smtpH")) SMTP_HOST_NAME = tokens[1];
			else if (l.startsWith("smtpU")) SMTP_AUTH_USER = tokens[1];
			else if (l.startsWith("smtpP")) SMTP_AUTH_PWD = tokens[1];
			else if (l.startsWith("from")) fromEmailAddress = tokens[1];
		}
